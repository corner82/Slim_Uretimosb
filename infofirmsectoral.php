 ...
 * @since 30-05-2016
 */
$app->get("/pkGetAll_infoFirmSectoral/", function () use ($app ) {
    $stripper = $app->getServiceManager()->get('filterChainerCustom');
    $stripChainerFactory = new \Services\Filter\Helper\FilterChainerFactory();
    $BLL = $app->getBLLManager()->get('infoFirmSectoralBLL');
    $headerParams = $app->request()->headers();
    if (!isset($headerParams['X-Public']))
        throw new Exception('rest api "pkGetAll_infoFirmSectoral" end point, X-Public variable not found');
    ...
    if (isset($resDataGrid['resultSet'][0]['id'])) {
        foreach ($resDataMenu as $menu) {
            $menus[] = array(
                ...
                "sector_id" => $menu["sector_id"],
                "sector_name" => $menu["sector_name"],
                "sector_name_eng" => $menu["sector_name_eng"],
...
* @since 30-05-2016
 */
$app->get("/pkDeletedAct_infoFirmSectoral/", function () use ($app ) {
    $stripper = $app->getServiceManager()->get('filterChainerCustom');
    $stripChainerFactory = new \Services\Filter\Helper\FilterChainerFactory();    
    $BLL = $app->getBLLManager()->get('infoFirmSectoralBLL');
     $headerParams = $app->request()->headers();
    if (!isset($headerParams['X-Public']))
        throw new Exception('rest api "pkDeletedAct_infoFirmSectoral" end point, X-Public variable not found');
    $pk = $headerParams['X-Public'];
    $vId = NULL;
    if (isset($_GET['id'])) {
...
* @since 30-05-2016
 */
$app->get("/pkUpdate_infoFirmSectoral/", function () use ($app ) {    
    $stripper = $app->getServiceManager()->get('filterChainerCustom');
    $stripChainerFactory = new \Services\Filter\Helper\FilterChainerFactory();   
    $BLL = $app->getBLLManager()->get('infoFirmSectoralBLL');   
    $headerParams = $app->request()->headers();
    if (!isset($headerParams['X-Public']))
        throw new Exception('rest api "pkUpdate_infoFirmSectoral" end point, X-Public variable not found');
    $pk = $headerParams['X-Public'];        
    $vLanguageCode = 'tr';
    if (isset($_GET['language_code'])) {
         $stripper->offsetSet('language_code',$stripChainerFactory->get(stripChainers::FILTER_ONLY_LANGUAGE_CODE,
...
* @since 30-05-2016
 */
$app->get("/pkInsert_infoFirmSectoral/", function () use ($app ) {    
    $stripper = $app->getServiceManager()->get('filterChainerCustom');
    $stripChainerFactory = new \Services\Filter\Helper\FilterChainerFactory();   
    $BLL = $app->getBLLManager()->get('infoFirmSectoralBLL');   
    $headerParams = $app->request()->headers();
    if (!isset($headerParams['X-Public']))
        throw new Exception('rest api "pkInsert_infoFirmSectoral" end point, X-Public variable not found');
    $pk = $headerParams['X-Public'];          
    $vLanguageCode = 'tr';
    if (isset($_GET['language_code'])) {
         $stripper->offsetSet('language_code',$stripChainerFactory->get(stripChainers::FILTER_ONLY_LANGUAGE_CODE,
...
* @since 30-05-2016
 */
$app->get("/pkFillFirmSectorNpk_infoFirmSectoral/", function () use ($app ) {
    $stripper = $app->getServiceManager()->get('filterChainerCustom');
    $stripChainerFactory = new \Services\Filter\Helper\FilterChainerFactory();
    $BLL = $app->getBLLManager()->get('infoFirmSectoralBLL');
    ...
    $headerParams = $app->request()->headers();
    if (!isset($headerParams['X-Public']))
        throw new Exception('rest api "pkFillFirmCertificateNpk_infoFirmSectoral" end point, X-Public variable not found');
    $pk = $headerParams['X-Public'];
    ...
    $resDataGrid = $BLL->fillFirmSectorNpk(array(
        'language_code' => $vLanguageCode,
        'network_key' => $vNpk,
        'pk' => $pk,
    ));
   
    $resTotalRowCount = $BLL->fillFirmSectorNpkRtc(array(
        'network_key' => $vNpk,
        'pk' => $pk,
    ));
    $counts=0;
 ...
 * @since 27-05-2016
 */
$app->get("/FillFirmSectorNpkQuest_infoFirmSectoral/", function () use ($app ) {
    $stripper = $app->getServiceManager()->get('filterChainerCustom');
    $stripChainerFactory = new \Services\Filter\Helper\FilterChainerFactory();
    $BLL = $app->getBLLManager()->get('infoFirmSectoralBLL');  
    ...
    $resDataGrid = $BLL->fillFirmSectorNpkQuest(array(
        'language_code' => $vLanguageCode,
        'network_key' => $vNpk,
