 ...
 * @since 15.06.2016
 */
class SysCustomerCriterion extends \DAL\DalSlim {

    /**
     * @author Okan CIRAN
     * @ sys_customer_criterion tablosundan parametre olarak  gelen id kaydını siler. !!
     * @version v 1.0  15.06.2016
     * @param array $params
     * @return array
     * @throws \PDOException
     */
    ...
    /**
     * @author Okan CIRAN
     * @ sys_customer_criterion tablosundaki tüm kayıtları getirir.  !!
     * @version v 1.0  15.06.2016  
     * @param array $params
     * @return array
     * @throws \PDOException
    ...
    public function getAll($params = array()) {
        try {
            ...
            $statement = $pdo->prepare("              
                SELECT 
                        a.id,
                        COALESCE(NULLIF(su.name, ''), a.name_eng) AS criterion,
                        a.name_eng,	
			...
			COALESCE(NULLIF(lx.language, ''), l.language_eng) AS language_name,			 
                ...
                LEFT JOIN sys_customer_criterion su ON (su.id = a.id OR su.language_parent_id = a.id) AND su.deleted =0 AND su.active =0 AND lx.id = su.language_id
    ...
    /**
     * @author Okan CIRAN
     * @ sys_customer_criterion tablosuna yeni bir kayıt oluşturur.  !!
     * @version v 1.0  15.06.2016
     * @param type $params
     * @return array
     * @throws \PDOException
        ...
        try {
            ...
            if (\Utill\Dal\Helper::haveRecord($opUserId)) {
                ...
                if (!\Utill\Dal\Helper::haveRecord($kontrol)) {
                    ...
                    $sql = "
                INSERT INTO sys_customer_criterion(                        
                        name, 
                        name_eng, 
                        link, 
                        logo, 
                ...
                VALUES (
                    ...
                    $insertID = $pdo->lastInsertId('sys_customer_criterion_id_seq');
                    $errorInfo = $statement->errorInfo();
                    if ($errorInfo[0] != "00000" && $errorInfo[1] != NULL && $errorInfo[2] != NULL)
                        throw new \PDOException($errorInfo[0]);
                    $pdo->commit();
    ...
    /**
     * @author Okan CIRAN
     * @ sys_customer_criterion tablosunda property_name daha önce kaydedilmiş mi ?  
     * @version v 1.0 15.06.2016
     * @param type $params
     * @return array
     * @throws \PDOException
    ...
    /**
     * @author Okan CIRAN
     * sys_customer_criterion tablosuna parametre olarak gelen id deki kaydın bilgilerini günceller   !!
     * @version v 1.0  15.06.2016
     * @param type $params
     * @return array
     * @throws \PDOException
    ...
    /**
     * @author Okan CIRAN
     * @ Gridi doldurmak için sys_customer_criterion tablosundan kayıtları döndürür !!
     * @version v 1.0  15.06.2016
    ...
    /**     
     * @author Okan CIRAN
     * @ Gridi doldurmak için sys_customer_criterion tablosundan çekilen kayıtlarının kaç tane olduğunu döndürür   !!
     * @version v 1.0  15.06.2016
     * @param array | null $args
     * @return array
     * @throws \PDOException
    ...
    /** 
     * @author Okan CIRAN
     * @ sosyal medya bilgilerini dropdown ya da tree ye doldurmak için sys_customer_criterion tablosundan kayıtları döndürür !!
     * @version v 1.0  15.06.2016
     * @param array | null $args
     * @return array
     * @throws \PDOException 
     */
    public function fillCustomerCriterionDdList($params = array()) {
        try {
            $pdo = $this->slimApp->getServiceManager()->get('pgConnectFactory');         
            $languageCode = 'tr';
            $languageIdValue = 647;
            ...
            $sql = "  
                ...
                INNER JOIN sys_language l ON l.id = a.language_id AND l.deleted =0 AND l.active =0  
		LEFT JOIN sys_language lx ON lx.id =" . intval($languageIdValue). " AND lx.deleted =0 AND lx.active =0                      		
                LEFT JOIN sys_customer_criterion sd ON (sd.id =a.id OR sd.language_parent_id = a.id) AND sd.deleted =0 AND sd.active =0 AND lx.id = sd.language_id   
                WHERE                    
                    a.deleted = 0 AND
     ...
     * @author Okan CIRAN
     * @ sys_customer_criterion tablosundan parametre olarak  gelen id kaydın aktifliğini
