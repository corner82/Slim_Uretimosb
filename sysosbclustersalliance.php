...
$app->get("/pkFillClustersAllianceLists_sysOsbClustersAlliance/", function () use ($app ) {
    ...
    $resData = $BLL->fillClustersAllianceLists(array(
    ...
    $menus[] = array("text" => "Lütfen Seçiniz", "value" => 0, "selected" => true, "imageSrc" => "", "description" => "Lütfen Seçiniz",); 
     if ($componentType == 'bootstrap') {
        ...
        foreach ($resData as $menu) {
            $menus[] = array(
                "id" => $menu["id"],       
                "text" => $menu["alliance_name"],
                "state" => $menu["state_type"],
                "checked" => false,
                "attributes" => array("notroot" => true, 
                                    "active" => $menu["active"] ,
                                    "alliance_type_name"=>$menu["alliance_type_name"],
                )                
            );
        }
        ...
        foreach ($resData as $menu) {
            $menus[] = array(
                ...
                "selected" => false,
                "description" => $menu["alliance_type_name"],
             //   "imageSrc" => ""
            );
        }
    }
 ...
 * @since 21-06-2016
 */
$app->get("/pkUpdateMakeActiveOrPassive_sysOsbClustersAlliance/", function () use ($app ) {
    $stripper = $app->getServiceManager()->get('filterChainerCustom');
    $stripChainerFactory = new \Services\Filter\Helper\FilterChainerFactory();    
    $BLL = $app->getBLLManager()->get('sysOsbClustersAllianceBLL');
    ...
    $headerParams = $app->request()->headers();
    if (!isset($headerParams['X-Public']))
        throw new Exception('rest api "pkUpdateMakeActiveOrPassive_sysOsbClustersAlliance" end point, X-Public variable not found');
    $pk = $headerParams['X-Public'];
